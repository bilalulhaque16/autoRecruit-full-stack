...
import { fieldExists, getUserRole, isNullObject } from "@src/utility/Utils"
...
import { loginRequest } from "@src/store/common/login/actions"
// ** Third Party Components
...
const defaultValues = {
  loginEmail: "testuser3@dummy.com",
  password: "pass123"
}
  ...
  useEffect(() => {
    if (fieldExists(loginStore, "error") && !isNullObject(loginStore.error)) {
      if (
        ...
        loginStore.error.message === "User is unauthorized"
      ) {
        dispatch(authActions.resendOtpRequest({ email: loginStore.email }))
      }
    }
  }, [loginStore.error])

  useEffect(() => {
    if (fieldExists(response, "status") && typeof response.data === "object") {
